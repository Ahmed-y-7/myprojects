    ...
    <div class="col-lg-8">
        ...
        <div class="card d-flex flex-row mt-3 p-4 align-items-center">
            ...
            <div class="mr-auto">
                <form action="/projects/{{$project->id}}/tasks/{{$task->id}}" method="POST">
                    @csrf
                    @method("PATCH")
                    <input type="checkbox" name="done" class="form-control ml-1" {{$task->done ? 'checked' :''}} onchange="this.form.submit()">
                </form>
            </div>
        ...
        <div class="card mt-4">
            <form action="/projects/{{$project->id}}/tasks" method="POST" class="d-flex p-3">
                @csrf
                <input type="text" name="body" class="form-control p-2 ml-2 border-0" placeholder="اضافة مهمة جديدة">
